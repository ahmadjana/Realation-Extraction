...
class GRU:
    def __init__(self, is_training, word_embeddings, settings):
        ...
        gru_cell_forward = tf.compat.v1.nn.rnn_cell.GRUCell(gru_size)
        gru_cell_backward = tf.compat.v1.nn.rnn_cell.GRUCell(gru_size)
        ...
        tf.summary.scalar('loss', self.total_loss)
        # regularization
        #self.l2_loss = tf.keras.regularizers.L1(0.01)
		#tf.contrib.layers.apply_regularization(regularizer=tf.contrib.layers.apply_regularization(0.0001),
                                                              #weights_list=tf.trainable_variables())
        self.final_loss = self.total_loss + self.total_loss
